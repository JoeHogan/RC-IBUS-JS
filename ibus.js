class IBus {
  ...
  constructor(config) {
    ...
    this.arrayBuffer = new ArrayBuffer(32);
  ...
  encode(max) { //encode channel data to array buffer. optional 'max' value can be used to skip channels that arent used.
    ...
    let checksum = this.initialChecksumValue - Array.from({length: 28}).reduce((total, cur, i) => {
      total += this.view.getUint8(i+2);
      return total;
    }, 0);
    this.view.setUint16(this.arrayBuffer.byteLength - 2, checksum, true);
    this.buffer = Buffer.from(this.arrayBuffer);
    return this.buffer;
  }
